...
`.trim()}(e,t),a=await fetch("https://ai-gateway.vercel.sh/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${o.O.VERCEL_AI_GATEWAY_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({model:"anthropic/claude-sonnet-4",max_tokens:4e3,messages:[{role:"system",content:s.a},{role:"user",content:n}]})});if(!a.ok){let e=await a.text();throw Error(`Vercel AI Gateway error: ${a.status} ${a.statusText} - ${e}`)}let i=await a.json(),l=i?.choices?.[0]?.message?.content??i?.choices?.[0]?.text??("string"==typeof i?i:"");if(!l||0===l.trim().length)throw Error(`Unexpected response format from Vercel AI Gateway: ${JSON.stringify(i).substring(0,500)}`);let c=function(e,t,n){let a=e,o=e.match(/```json\s*([\s\S]*?)\s*```/)||e.match(/\{[\s\S]*\}/);o&&(a=o[1]||o[0]);try{let e=JSON.parse(a),o=(Array.isArray(e.selected_contracts)?e.selected_contracts:[]).map(e=>{let n=t.find(t=>t.market_id===e.market_id);if(!n)throw Error(`Contract not found: ${e.market_id}`);return{contract:n,allocation:Math.min(Math.max(e.allocation,20),50),confidence:Math.min(Math.max(e.confidence,0),1),reasoning:e.reasoning||"No reasoning provided",riskFactors:e.risk_factors||[]}});o.length>3&&(o=o.slice(0,3));let s=o.reduce((e,t)=>e+t.allocation,0);if(s>n&&s>0){let e=n/s,t=(o=o.map(t=>({...t,allocation:Math.round(t.allocation*e*100)/100}))).reduce((e,t)=>e+t.allocation,0);t>n&&o.length>0&&(o[o.length-1].allocation-=t-n,o[o.length-1].allocation=Math.max(0,Math.round(100*o[o.length-1].allocation)/100))}return{selectedContracts:o,totalAllocated:o.reduce((e,t)=>e+t.allocation,0),strategyNotes:e.strategy_notes||"No strategy notes"}}catch(t){throw console.error("Failed to parse AI response:",t),console.error("Response text:",e),Error(`Failed to parse AI response: ${t.message}`)}}(l,e.contracts,e.dailyBudget);return console.log(`   ✅ AI selected ${c.selectedContracts.length} contracts`),console.log(`   💰 Total allocation: $${c.totalAllocated.toFixed(2)}`),c}[o,r]=i.then?(await i)():i,a()}catch(e){a(e)}})},4777:(e,t,n)=>{n.a(e,async(e,a)=>{try{n.d(t,{T:()=>r});var o=n(3435),s=e([o]);async function r(){let e=(await (0,o.lv)(100)).filter(e=>"open"!==e.status);if(0===e.length)return"No historical trades yet. This is a fresh start.";let t=0===e.length?0:e.filter(e=>"won"===e.status).length/e.length,n=i(e),a=e.reduce((e,t)=>e+(t.pnl||0),0),s=e.filter(e=>"won"===e.status),r=e.filter(e=>"lost"===e.status||"stopped"===e.status),c=function(e,t){let n=[],a=t.filter(e=>e.ai_confidence>=.9),o=a.filter(e=>"won"===e.status);a.length>0&&n.push({pattern:"Very High Confidence (≥90%)",winRate:o.length/a.length,avgROI:i(o),example:o[0]?.ai_reasoning?.substring(0,80)});let s=t.filter(e=>e.ai_confidence>=.85&&e.ai_confidence<.9),r=s.filter(e=>"won"===e.status);s.length>0&&n.push({pattern:"High Confidence (85-90%)",winRate:r.length/s.length,avgROI:i(r),example:r[0]?.ai_reasoning?.substring(0,80)});let l=t.filter(e=>{let t=e.contract?.end_date?new Date(e.contract.end_date):null;return!!t&&(t.getTime()-new Date(e.executed_at).getTime())/864e5<=1}),c=l.filter(e=>"won"===e.status);l.length>0&&n.push({pattern:"Short-term Contracts (≤1 day to resolution)",winRate:c.length/l.length,avgROI:i(c),example:c[0]?.contract?.question?.substring(0,80)});let d=t.filter(e=>e.entry_odds>=.92),u=d.filter(e=>"won"===e.status);return d.length>0&&n.push({pattern:"Very High Entry Odds (≥92%)",winRate:u.length/d.length,avgROI:i(u),example:u[0]?.ai_reasoning?.substring(0,80)}),n.sort((e,t)=>t.winRate-e.winRate)}(0,e),d=function(e){let t=[];if(0===e.length)return t;let n=e.filter(e=>e.ai_confidence>=.9);n.length>0&&t.push({pattern:"Overconfident Trades (≥90% confidence that lost)",count:n.length,totalLoss:n.reduce((e,t)=>e+Math.abs(t.pnl||0),0),avgLoss:n.reduce((e,t)=>e+Math.abs(t.pnl||0),0)/n.length,example:n[0]?.ai_reasoning?.substring(0,80)});let a=e.filter(e=>{let t=e.contract?.end_date?new Date(e.contract.end_date):null;return!!t&&(t.getTime()-new Date(e.executed_at).getTime())/864e5>1.5});a.length>0&&t.push({pattern:"Longer-term Contracts (>1.5 days) that Lost",count:a.length,totalLoss:a.reduce((e,t)=>e+Math.abs(t.pnl||0),0),avgLoss:a.reduce((e,t)=>e+Math.abs(t.pnl||0),0)/a.length,example:a[0]?.ai_reasoning?.substring(0,80)});let o=e.filter(e=>"stopped"===e.status);o.length>0&&t.push({pattern:"Stop Loss Triggered",count:o.length,totalLoss:o.reduce((e,t)=>e+Math.abs(t.pnl||0),0),avgLoss:o.reduce((e,t)=>e+Math.abs(t.pnl||0),0)/o.length,example:o[0]?.ai_reasoning?.substring(0,80)});let s=e.filter(e=>e.entry_odds>=.85&&e.entry_odds<.92);return s.length>0&&t.push({pattern:"Mid-range Odds (85-92%) that Lost",count:s.length,totalLoss:s.reduce((e,t)=>e+Math.abs(t.pnl||0),0),avgLoss:s.reduce((e,t)=>e+Math.abs(t.pnl||0),0)/s.length,example:s[0]?.ai_reasoning?.substring(0,80)}),t.sort((e,t)=>t.totalLoss-e.totalLoss)}(r),u=function(e){let t=new Map;return e.forEach(e=>{let n=e.contract?.question||"Unknown",a="Other";n.toLowerCase().includes("election")||n.toLowerCase().includes("vote")?a="Elections/Politics":n.toLowerCase().includes("earnings")||n.toLowerCase().includes("stock")?a="Earnings/Stocks":n.toLowerCase().includes("data")||n.toLowerCase().includes("release")?a="Data Releases":n.toLowerCase().includes("deadline")||n.toLowerCase().includes("date")?a="Time-based":(n.toLowerCase().includes("approval")||n.toLowerCase().includes("approve"))&&(a="Approval/Regulatory"),t.has(a)||t.set(a,{wins:0,losses:0});let o=t.get(a);"won"===e.status?o.wins++:("lost"===e.status||"stopped"===e.status)&&o.losses++}),Array.from(t.entries()).map(([e,t])=>({type:e,wins:t.wins,losses:t.losses,winRate:t.wins+t.losses>0?t.wins/(t.wins+t.losses):0})).sort((e,t)=>t.wins+t.losses-(e.wins+e.losses))}(e),h=[{min:.9,max:1,label:"90-100%"},{min:.85,max:.9,label:"85-90%"},{min:.8,max:.85,label:"80-85%"},{min:.7,max:.8,label:"70-80%"}].map(t=>{let n=e.filter(e=>e.ai_confidence>=t.min&&e.ai_confidence<t.max),a=n.filter(e=>"won"===e.status),o=n.filter(e=>"lost"===e.status||"stopped"===e.status);return{range:t.label,wins:a.length,losses:o.length,winRate:n.length>0?a.length/n.length:0,avgROI:i(n)}}),g=function(e){if(0===e.length)return"";let t=e.map(e=>{let t="won"===e.status?"✅ WIN":"lost"===e.status?"❌ LOSS":"⚠️ STOPPED",n=e.pnl?e.pnl>=0?`+$${e.pnl.toFixed(2)}`:`-$${Math.abs(e.pnl).toFixed(2)}`:"N/A",a=e.pnl&&e.position_size?`${(e.pnl/e.position_size*100).toFixed(1)}%`:"N/A",o=`${(100*e.ai_confidence).toFixed(0)}%`,s=new Date(e.executed_at).toLocaleDateString("en-US",{month:"short",day:"numeric"}),r=e.contract?.question||"N/A",i=e.ai_reasoning||"No reasoning recorded";return`
${t} | ${s} | Confidence: ${o} | P&L: ${n} (${a})
...
LESSONS LEARNED:
${(function(e,t){let n=[];if(e.length>0&&t.length>0){let a=e.reduce((e,t)=>e+t.ai_confidence,0)/e.length,o=t.reduce((e,t)=>e+t.ai_confidence,0)/t.length;a<o&&n.push('Lower confidence trades actually performed better - be more cautious with "sure things"');let s=e.map(e=>{let t=e.contract?.end_date?new Date(e.contract.end_date):null;return t?(t.getTime()-new Date(e.executed_at).getTime())/864e5:null}).filter(e=>null!==e),r=t.map(e=>{let t=e.contract?.end_date?new Date(e.contract.end_date):null;return t?(t.getTime()-new Date(e.executed_at).getTime())/864e5:null}).filter(e=>null!==e);if(s.length>0&&r.length>0){let e=s.reduce((e,t)=>e+t,0)/s.length,t=r.reduce((e,t)=>e+t,0)/r.length;e<t&&n.push(`Shorter-term contracts (avg ${e.toFixed(1)} days) performed better than longer-term (avg ${t.toFixed(1)} days)`)}}let a=l(e.map(e=>e.ai_reasoning||"").join(" ")),o=l(t.map(e=>e.ai_reasoning||"").join(" ")),s=a.filter(e=>!o.includes(e));return s.length>0&&n.push(`Winning trades often mentioned: ${s.slice(0,3).join(", ")}`),0===n.length&&n.push("Continue to be conservative and focus on high-probability, low-variance contracts"),n.map((e,t)=>`${t+1}. ${e}`).join("\n")})(s,r)}
  `.trim()}function i(e){if(0===e.length)return 0;let t=e.filter(e=>null!==e.pnl&&e.position_size>0);return 0===t.length?0:t.reduce((e,t)=>e+t.pnl/t.position_size,0)/t.length}function l(e){let t=e.toLowerCase();return["clear","objective","deadline","scheduled","guaranteed","certain","volatile","unpredictable","surprise","uncertain","risk","black swan"].filter(e=>t.includes(e))}o=(s.then?(await s)():s)[0],a()}catch(e){a(e)}})},1674:(e,t,n)=>{n.d(t,{a:()=>a});let a=`
...
`},8231:(e,t,n)=>{n.a(e,async(e,a)=>{try{n.d(t,{O:()=>i});var o=n(1309),s=n(8159),r=e([o,s]);[o,s]=r.then?(await r)():r;let i=(0,o.createClient)(s.O.SUPABASE_URL,s.O.SUPABASE_KEY);a()}catch(e){a(e)}})},3435:(e,t,n)=>{n.a(e,async(e,a)=>{try{n.d(t,{$r:()=>l,FC:()=>m,Jg:()=>u,Of:()=>f,Pq:()=>c,Xb:()=>d,Xx:()=>p,dY:()=>g,fT:()=>y,lv:()=>i,mo:()=>r,sl:()=>h,zC:()=>_});var o=n(8231),s=e([o]);async function r(){let{data:e,error:t}=await o.O.from("trades").select(`
      *,
      contract:contracts(*)
    ...
    `).single();if(n)throw n;return t}async function u(e,t){let{data:n,error:a}=await o.O.from("trades").update(t).eq("id",e).select(`
      *,
      contract:contracts(*)
    `).single();if(a)throw a;return n}async function h(){return(await r()).map(e=>({trade:e,current_odds:e.entry_odds,unrealized_pnl:0,unrealized_pnl_pct:0}))}async function g(){let{data:e}=await o.O.from("performance_metrics").select("bankroll").order("date",{ascending:!1}).limit(1).single();return e?.bankroll||Number(process.env.INITIAL_BANKROLL)||1e3}async function m(){return Number(process.env.INITIAL_BANKROLL)||1e3}async function p(e){let{data:t}=await o.O.from("performance_metrics").select("bankroll").lte("date",e.toISOString()).order("date",{ascending:!1}).limit(1).single();return t?.bankroll||Number(process.env.INITIAL_BANKROLL)||1e3}async function f(){let e=await g(),t=(await h()).reduce((e,t)=>e+t.trade.position_size,0);return e-t}async function y(){let{data:e,error:t}=await o.O.from("notification_preferences").select("*").eq("user_id","default").single();if(t&&"PGRST116"!==t.code)throw t;return e||{enabled:!1}}async function _(e){let t=new Date(Date.now()-36e5*e),{data:n,error:a}=await o.O.from("stop_loss_events").select(`
      *,
      trade:trades(
        *,
        contract:contracts(*)
      )
    `).gte("executed_at",t.toISOString()).order("executed_at",{ascending:!1});if(a)throw a;return n}o=(s.then?(await s)():s)[0],a()}catch(e){a(e)}})},5526:(e,t,n)=>{n.a(e,async(e,a)=>{try{n.d(t,{A0:()=>c,US:()=>i});var o=n(8231),s=n(913),r=e([o,s]);async function i(){let e=new Date(Date.now()-72e5),{data:t,error:n}=await o.O.from("contracts").select("*").gte("discovered_at",e.toISOString()).eq("resolved",!1).order("discovered_at",{ascending:!1});return n?(console.error("Error fetching cached markets:",n),[]):t&&0!==t.length?t.map(e=>({market_id:e.market_id,question:e.question,end_date:new Date(e.end_date),yes_odds:parseFloat(e.current_odds.toString()),no_odds:1-parseFloat(e.current_odds.toString()),liquidity:parseFloat(e.liquidity?.toString()||"0"),volume_24h:parseFloat(e.volume_24h?.toString()||"0"),resolved:e.resolved||!1,category:e.category||void 0,outcome:e.outcome||void 0,final_odds:e.final_odds?parseFloat(e.final_odds.toString()):void 0,resolved_at:e.resolved_at?new Date(e.resolved_at):void 0})):[]}async function l(e){if(0===e.length)return;console.log(`💾 Caching ${e.length} markets to database...`);let t=e.map(e=>({market_id:e.market_id,question:e.question,end_date:e.end_date.toISOString(),current_odds:e.yes_odds,category:e.category||null,liquidity:e.liquidity||0,volume_24h:e.volume_24h||0,resolved:e.resolved||!1,outcome:e.outcome||null,final_odds:e.final_odds||null,resolved_at:e.resolved_at?.toISOString()||null,discovered_at:new Date().toISOString()}));for(let e=0;e<t.length;e+=100){let n=t.slice(e,e+100),{error:a}=await o.O.from("contracts").upsert(n,{onConflict:"market_id",ignoreDuplicates:!1});a?console.error(`Error caching markets chunk ${e/100+1}:`,a):console.log(`   ✅ Cached chunk ${e/100+1}/${Math.ceil(t.length/100)} (${n.length} markets)`)}console.log(`✅ Cached ${e.length} markets successfully`)}async function c(e){console.log("\uD83D\uDD04 Refreshing market page...",e?`(cursor: ${e.substring(0,20)}...)`:"(first page)");let t=(0,s.DZ)();try{let n=await t.getMarkets(100,e||void 0,void 0,void 0,void 0,void 0,void 0,void 0,void 0,void 0,"open",void 0,void 0),a=n.data.markets||[],o=n.data.cursor||null,r=a.map(e=>{let t;let n=(0,s.TN)(e);try{let n=e.expected_expiration_time||e.expiration_time||e.expirationTime||e.close_time||e.end_date;t=new Date(n),isNaN(t.getTime())&&(t=new Date(Date.now()+6048e5))}catch(e){t=new Date(Date.now()+6048e5)}let a="closed"===e.status||"settled"===e.status||"resolved"===e.status,o="yes"===e.result?"YES":"no"===e.result?"NO":void 0;return{market_id:e.ticker||e.market_id||e.id,question:e.title||e.question||e.subtitle||e.yes_sub_title||"N/A",end_date:t,yes_odds:null!==n?n/100:0,no_odds:null!==n?(100-n)/100:0,liquidity:0,volume_24h:parseFloat(e.volume_24h||e.volume||0),resolved:a,category:e.category||e.event_ticker||void 0,outcome:o,final_odds:e.last_price?parseFloat(e.last_price.toString())/100:void 0,resolved_at:e.close_time?new Date(e.close_time):void 0}});return await l(r),console.log(`   ✅ Cached ${r.length} markets. Next cursor: ${o?"yes":"no"}`),{markets:r,nextCursor:o,isComplete:!o||0===a.length}}catch(n){if(n.response?.status===429){let e=n.response.headers["retry-after"],t=e?1e3*parseInt(e):5e3;throw Error(`Rate limited. Wait ${t}ms`)}if(n.message?.startsWith("RATE_LIMITED:")){let e=parseInt(n.message.split(":")[1]);throw Error(`Rate limited. Wait ${e}ms`)}let e=n.response?.data?.error?.message||n.message||"Unknown error",t=n.response?.status||500;throw Error(`Kalshi API error: ${t} - ${e}`)}}[o,s]=r.then?(await r)():r,a()}catch(e){a(e)}})},7372:(e,t,n)=>{n.a(e,async(e,a)=>{try{n.d(t,{w:()=>l});var o=n(913),s=n(5526),r=n(3365),i=e([o,s]);async function l(e={minOdds:r.U.MIN_ODDS,maxOdds:r.U.MAX_ODDS,maxDaysToResolution:r.U.MAX_DAYS_TO_RESOLUTION,minLiquidity:r.U.MIN_LIQUIDITY,excludeCategories:r.U.EXCLUDE_CATEGORIES,excludeKeywords:r.U.EXCLUDE_KEYWORDS}){console.log("\uD83D\uDD0D Scanning Kalshi for high-conviction contracts..."),console.log(`   Criteria: ${100*e.minOdds}%-${100*e.maxOdds}% odds, <${e.maxDaysToResolution} days, >$${e.minLiquidity} liquidity`);let t=await (0,s.US)();if(console.log(`   ✅ Retrieved ${t.length} markets from cache`),0===t.length)return console.warn("⚠️ No cached markets found. Market refresh cron may not have run yet."),[];let n=[];for(let a of t){if(!a.yes_odds||0===a.yes_odds||null===a.yes_odds)continue;let t=100*a.yes_odds,s=t>=100*e.minOdds,r=t<=(1-e.maxOdds)*100;if(!s&&!r)continue;let i=(0,o.YI)(a.end_date);if(i>e.maxDaysToResolution||i<0||a.resolved||a.category&&e.excludeCategories?.includes(a.category))continue;let l=a.question.toLowerCase();(e.excludeKeywords||[]).map(e=>e.toLowerCase()).some(e=>l.includes(e))||n.push(a)}console.log(`   📊 Found ${n.length} high-conviction candidates after filtering`);let a=[],i=[];for(let t=0;t<n.length;t++){let s=n[t];try{let{liquidity:r,side:i}=await (0,o.Z9)(s.market_id);if(r<e.minLiquidity)continue;let l={id:"",market_id:s.market_id,question:s.question,end_date:s.end_date,current_odds:s.yes_odds,liquidity:r,volume_24h:s.volume_24h,category:s.category,discovered_at:new Date};a.push(l),((t+1)%10==0||t+1===n.length)&&console.log(`   📈 Enriched ${t+1}/${n.length} candidates... (${a.length} passed liquidity filter)`)}catch(e){i.push(`${s.market_id}: ${e.message}`);continue}}return i.length>0&&(console.warn(`   ⚠️ ${i.length} markets failed enrichment (likely resolved or inactive)`),i.length<=5&&i.forEach(e=>console.warn(`      ${e}`))),a.sort((e,t)=>(t.liquidity||0)-(e.liquidity||0)),console.log(`   ✅ Found ${a.length} qualifying contracts with sufficient liquidity`),a}[o,s]=i.then?(await i)():i,a()}catch(e){a(e)}})}};